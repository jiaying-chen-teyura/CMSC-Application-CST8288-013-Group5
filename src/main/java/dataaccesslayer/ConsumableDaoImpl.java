...
/**
 * JDBC implementation of the consumable persistence operations.
 * Provides CRUD-style access and inventory-specific reporting for consumables.
...
public class ConsumableDaoImpl implements ConsumableDao {
    ...
    /**
     * Retrieves the materials reported against a usage session, with the material name/unit
     * joined in so the FR-03 live session report and checkout receipt don't need N+1 lookups.
     * @param usageSessionId the ID of the session
     * @return the MaterialUsageDTOs recorded for that session
     */
    @Override
    public List<MaterialUsageDTO> getMaterialUsageForSession(int usageSessionId) {
        String sql = "SELECT m.*, c.material_name, c.unit FROM material_usage m "
                + "JOIN consumables c ON c.consumable_id = m.consumable_id "
                + "WHERE m.usage_session_id = ?";
