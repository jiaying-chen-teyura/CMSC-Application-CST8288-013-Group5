...
/**
 * JDBC implementation of work-order persistence operations.
 * Provides methods for creating, tracking, and updating work orders through the database.
...
public class WorkOrderDaoImpl implements WorkOrderDao {
    ...
    private WorkOrderDTO map(ResultSet rs) throws SQLException {
        ...
        if (started != null) w.setStartedAt(started.toLocalDateTime());
    ...
    public void startWorkOrder(int workOrderId) {
        String sql = "UPDATE work_orders SET status='IN_PROGRESS', started_at=NOW() "
                + "WHERE work_order_id=? AND status='ACCEPTED'";
        try (Connection con = DataSource.getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setInt(1, workOrderId);
            ps.executeUpdate();
        } catch (SQLException e) {
            throw new RuntimeException("startWorkOrder failed", e);
        }
    }
