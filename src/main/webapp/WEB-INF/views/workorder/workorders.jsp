                ...
                <tbody>
                  <c:forEach var="w" items="${openWorkOrders}">
                    <tr>
                      ...
                      <td>
                        <c:choose>
                          <c:when test="${not empty w.quotedPrice}">$${w.quotedPrice}</c:when>
                          <c:otherwise>$${w.estimatedLabourCost}</c:otherwise>
                        </c:choose>
                      </td>
                      <td>
                        ...
                        <c:if test="${w.status == 'ACCEPTED'}">
                          <span class="text-muted">Start it below</span>
                        </c:if>
                        <c:if test="${w.status == 'IN_PROGRESS'}">
                          <span class="text-muted">In progress &ndash; complete it below</span>
                        </c:if>
                      </td>
                    </tr>
                  </c:forEach>
                  ...
                  <c:forEach var="w" items="${myWorkOrders}">
                    <tr>
                      ...
                      <td>
                        <c:if test="${w.status == 'ACCEPTED'}">
                          <form style="display:inline" action="${pageContext.request.contextPath}/controller" method="POST">
                            <input type="hidden" name="action" value="startWorkOrder">
                            <input type="hidden" name="workOrderId" value="${w.workOrderId}">
